...
class JsonFormatter {
  ...
  def toUnderCollectionJson(current: ObjectProperty, elements: Seq[ObjectProperty], users: Map[Int, User], parents: Seq[Parent]): Option[JsValue] = {
    ...
    Some(Json.toJson(Elements(parents, currentFolder, elementList)))
  }
